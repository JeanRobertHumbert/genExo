...
\runningfootrule
\lhead{Nom : \\ Prénom :}
\chead{Devoir Commun 17 Février 2022\small{Correction - genCode : 3953286104}}
\rhead{TSTMG}
...
% Document info
\textsc{\Large Devoir Commun 17 Février 2022Correction\\ \small{genCode : 3953286104}}\\[5cm]
			...
			\begin{parts}
                    ...
                    \begin{solution}
                        \begin{center}
                            \begin{tikzpicture}[xscale=1,yscale=1]
                                ...
                                \draw[fleche] (R)--(Ra) node[etiquette] {$65\%$};
                                \draw[fleche] (Ra)--(Raa) node[etiquette] {$44\%$};
                                \draw[fleche] (Ra)--(Rab) node[etiquette] {$56\%$};
                                \draw[fleche] (R)--(Rb) node[etiquette] {$35\%$};
                                \draw[fleche] (Rb)--(Rba) node[etiquette] {$42\%$};
                                \draw[fleche] (Rb)--(Rbb) node[etiquette] {$56\%$};
                            \end{tikzpicture}
                        \end{center}
                    \end{solution}
				\Part[1] Déterminer la valeur de la probabilité $p(T \cap L)$.
                    \begin{solution}
						$p(T\cap L)=p(T)\times p_T(L)=0.65\times 0.44=0.286$
					\end{solution}
				\Part[1] Montrer que $p(L) = 43.3\%$.
                    \begin{solution}
						$p(L) = p(T\cap L)+p(A\cap L) = 0.286+0.35\times 0.42=0.286+0.147=0.433$
					\end{solution}
				\Part[1] Déterminer la probabilité d'avoir suivi une formation en BTS ou DUT sachant que l'on a obtenu une licence. On arrondira le résultat à  $0,01$\,\%.
                    \begin{solution}
						...
						\begin{center}
							$p_L(T) =\dfrac{p(L\cap T)}{p(L)}=\dfrac{0.286}{0.433}\approx 0.6605$
						\end{center}
					\end{solution}
				\Part[2] Déterminer la valeur arrondie à  $0,01$\,\% de la probabilité $p_L(A)$. Interpréter.
                    \begin{solution}
						\begin{center}
							$p_L(A)=\dfrac{p(A\cap L)}{p(L)}=\dfrac{0.147}{43.3}\approx 0.0034$
						\end{center}
...
%					\fillwithlines{20mm}
					\begin{solution}
						En 2000, il y avait 7939 morts; en 2010 il y en a eu :
						\begin{center}
							$7939\times \left( 1-\dfrac{14}{100}\right)\approx 6827$
						\end{center}
					\end{solution}
			\end{parts}
\clearpage
...
%						\fillwithlines{30mm}
						\begin{solution}
							\begin{center}
								$\dfrac{pourcent_{2010}-pourcent_{2006}}{pourcent_{2006}}\times 100 = \dfrac{53.2-54.5}{54.5}\times 100 \approx -2.39$ soit -2\%
							\end{center}
						\end{solution}
					\Part[2] Déterminer le taux annuel moyen entre 2006 et 2010. On donnera le résultat en pourcentage arrondi au centième.
%						\fillwithlines{30mm}
						\begin{solution}
							Le taux global qui fait passer de 2006 à 2010 est de -2\%. Le taux moyen est donc :
							\begin{center}
								$t_m=\sqrt[4]{1+\dfrac{-2}{100}}-1\approx -0.5038\%$
							\end{center}
						\end{solution}
...
%						\fillwithlines{30mm}
						\begin{solution}
							De 2012 à 2020, il y a 8 ans, le taux en 2012 était de 52.5\%. Le taux en 2020 est donc :
							\begin{center}
								$52.5\times\left(1+\dfrac{4.91}{100}\right)^8$ soit environ 77.04\%
							\end{center}
						\end{solution}
				\end{parts}
\clearpage
    ...
    \Question
		...
		\begin{parts}
			...
			\begin{solution}
				...
				\begin{center}
					$C(x)=u+v+w+z$\\
					avec $u=\dfrac{1}{3}x^3$, $v=-7x^2$, $w=45x$ et $z=11$
				\end{center}
				$u$, $v$, $w$ et $z$ sont de la forme $kx^n$ or $(kx^n)'=knx^{n-1}$ on a donc :
				\begin{center}
					$C'(x)=u'+v'+w'+z'$\\
					avec $u'=1x^2$, $v'=-14x$, $w'=45$ et $z'=0$
				\end{center}
				d'où :
				\begin{center}
					$C'(x)=1x^2+-14x+45 +0$\\
			...
			\begin{solution}
				...
				\begin{align*}
					...
					x_1&=\dfrac{-\left(-14\right)-\sqrt{16}}{2\times+1}\\
					x_1&=\dfrac{-\left(-14\right)-4}{2\times+1}\\
					x_1&=5
				\end{align*}
				...
				\end{align*}
				\end{multicols}
				Les 2 solutions sont donc $x_1=5$ et $x_2=9$
			\end{solution}
			...
			\begin{solution}
				...
				\begin{center}
					\begin{tikzpicture}
						\tkzTabInit{$x$/1,$C'(x)$/1,$C$/3}{$-\infty$,$5$,$9$,$\infty$}
						\tkzTabLine{,+,z,-,z,+}
						\tkzTabVar{-/$-\infty$,+/$C\left(5\right)$,-/$C\left(9\right)$,+/$+\infty$}
					\end{tikzpicture}
				\end{center}
			\end{solution}
			\Part[3] Conclure sur la quantité optimale de production et en donner donc le coût minimal au kilogramme\\
			\begin{solution}
				La quantité optimale à produire est $9$ kilogrammes pour :
				\begin{align*}
					C(9)&=\dfrac{1}{3}\times \left(9\right)^3+-7\times \left(9\right)^2+45\times 9+11\\
						&=92
				\end{align*}
			\end{solution}
		\end{parts}
